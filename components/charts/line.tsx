    ...
    if (containerRef.current && data) {
      ...
      chart.subscribeCrosshairMove((param) => {
        if (param.time && tooltipRef.current && param.point && containerRef.current) {
          ...
          const formattedPrice = (price.value * xrpPrice).toLocaleString("en-us", {
            style: "currency",
            currency: "USD",
          ...
          tooltipRef.current.innerHTML = `<p>${formattedPrice}</p><p class="text-muted-foreground text-nowrap">${format(new Date(price.time * 1000), "MMM d hh:mm a")}</p>`;
        } else {
