...
export const Transactions = () => {
  ...
  const formatLabel = (tx: Transaction) => {
    const getCurrency = (amount: Amount) => {
      ...
      if (typeof amount === "object") {
        return amount.currency.length === 40 ? formatCurrency(amount.currency) : amount.currency;
      }
      return "";
    };
  ...
  const formatAmount = (tx: Transaction) => {
    ...
    if (typeof tx.amountDelivered === "object") {
      const currency =
        typeof tx.amountDelivered === "object" && "currency" in tx.amountDelivered
          ? tx.amountDelivered.currency.length === 40
            ? formatCurrency(tx.amountDelivered.currency)
        ...
        {transactions.map((tx: Transaction) => (
          <div key={tx.hash}>
            <a href={`https://xrpscan.com/tx/${tx.hash}`} target="_blank" rel="noopener noreferrer">
              <Card className="cursor-pointer p-0 transition-all hover:border-border dark:hover:bg-secondary/60">
